...
- **2026-06-16** — `src/render.py` + `src/publish.py` + `scripts/daily_post.py`: Metadata overhaul + custom thumbnails. Title now hook-first (`"You're wasting your life" — Seneca | Stoicism`) instead of `Day N | quote...` — the hook is the scroll-stopper for new viewers; day number moves to description for loyal fans. Tags expanded from hashtag list to 30-tag SEO set including author name, theme, and niche discovery terms. Custom 1080x1920 JPEG thumbnail generated per video (hook text + author on cinematic background) and uploaded via `thumbnails.set` API immediately after publish — appears in the Shorts shelf, channel grid, and search results before a viewer clicks.
