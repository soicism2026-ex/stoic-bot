...
# ---------------------------------------------------------------------------
# Standalone hook generation (used to backfill titles/thumbnails on existing
# videos, where only the stored quote/author/theme are available).
# ---------------------------------------------------------------------------
...
def generate_hook(quote: str, author: str, theme: str) -> str:
    """Generate a single 3-5 word scroll-stopping hook for an existing quote."""
    import anthropic
    client = anthropic.Anthropic(api_key=os.environ["ANTHROPIC_API_KEY"])
    msg = client.messages.create(
        model=MODEL,
